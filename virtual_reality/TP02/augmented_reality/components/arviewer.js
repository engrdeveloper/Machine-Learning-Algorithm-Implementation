...
ARViewer = ARCS.Component.create(
  ...
  function () {
    ...
    this.setWidgets = function (cName, vName) {
      ...
      sourceAspectRatio =
        parseInt(videoStyle.width) / parseInt(videoStyle.height);
    ...
    this.setIntrinsics = function (parameters) {
      ...
      var uZero = parameters[2];
      var alphaV = parameters[4];
      var vZero = parameters[5];

      var width = sourceAspectRatio * sourceHeight;
      var height = sourceHeight;

      var near = camera3d.near
      var far = camera3d.far;

      // compute frustum
      var left = (-uZero * near) / alphaU;
      var right = ((width - uZero) * near) / alphaU;
      var top = ((height-vZero) * near) / alphaV;
      var bottom = (-vZero * near) / alphaV;
    ...
    this.setExtrinsics = function (markers) {
      ...
      for (i = 0; i < markers.length; i++) {
        if (markers[i].id === sceneId) {
          ...
          const R = marker.pose.rotation; // 3x3 matrix
          ...
          // Invert it — because we want camera pose, not marker pose
          const cameraMatrix = new THREE.Matrix4();
          cameraMatrix.getInverse(markerMatrix);

          // Apply this transformation to the virtual camera
          camera3d.matrix.copy(cameraMatrix);
          camera3d.matrix.decompose(
            camera3d.position,
            camera3d.quaternion,
            camera3d.scale
          );
